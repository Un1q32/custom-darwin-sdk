 ...
 *#13876 25.01.2024  revert the previous patch (size_t instead of unsigned int)
 ...
 */
#define LLEN                                                                   \
  (2 * (int)sizeof(unsigned long) + 2 + /* addr + ": " */                      \
...
#define CONDITIONAL_CAPITALIZE(c)                                              \
  (capitalize ? toupper((unsigned char)(c)) : (c))
...
static void exit_with_usage(void) {
  ...
  fprintf(stderr, "    -r          reverse operation: convert (or patch) "
                  "hexdump into binary.\n");
  fprintf(stderr, "    -r -s off   revert with <off> added to file positions "
                  "found in hexdump.\n");
  ...
  fprintf(stderr, "    -R when     colorize the output; <when> can be "
...
static int huntype(FILE *fpi, FILE *fpo, int cols, int hextype, long base_off) {
  int c, ign_garb = 1, n1 = -1, n2 = 0, n3 = 0, p = cols, bt = 0, b = 0,
         bcnt = 0;
...
int main(int argc, char *argv[]) {
  ...
  while (argc >= 2) {
    ...
    if (!STRNCMP(pp, "-a", 2))
      ...
      if (pp[2] && STRNCMP("ffset", pp + 2, 5))
        ...
        if (negoffset)
          displayoff =
              ULONG_MAX - strtoul(argv[2] + reloffset + negoffset, NULL, 0) + 1;
        else
  ...
  if ((hextype == HEX_POSTSCRIPT && cols < 0) ||
      ...
      ((hextype == HEX_NORMAL || hextype == HEX_BITS ||
        hextype == HEX_LITTLEENDIAN) &&
       (cols > COLS))) {
  ...
  if (hextype == HEX_CINCLUDE) {
    ...
    if (varname != NULL) {
      FPRINTF_OR_DIE((fpo, "unsigned char %s",
                      isdigit((unsigned char)varname[0]) ? "__" : ""));
      for (e = 0; (c = varname[e]) != 0; e++)
        putc_or_die(isalnum((unsigned char)c) ? CONDITIONAL_CAPITALIZE(c) : '_',
                    fpo);
      fputs_or_die("[] = {\n", fpo);
    }
    ...
    if (varname != NULL) {
      fputs_or_die("};\n", fpo);
      FPRINTF_OR_DIE((fpo, "unsigned int %s",
                      isdigit((unsigned char)varname[0]) ? "__" : ""));
      for (e = 0; (c = varname[e]) != 0; e++)
        putc_or_die(isalnum((unsigned char)c) ? CONDITIONAL_CAPITALIZE(c) : '_',
                    fpo);
